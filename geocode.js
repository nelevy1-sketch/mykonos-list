...
async function geocodeQuery(name, language) {
  try {
    const response = await fetch(
      `https://nominatim.openstreetmap.org/search?q=${encodeURIComponent(name)}&format=json&limit=1&accept-language=${language}`
    );
...
window.geocodeName = async function geocodeName(name, options = {}) {
  ...
  return geocodeQuery(name, language);
};
